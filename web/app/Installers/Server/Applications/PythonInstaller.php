    ...
    public function install()
    {
        ...
        if ($packageManager->isDebianBased()) {
            ...
            foreach ($this->pythonVersions as $pythonVersion) {
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion);
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-dev');
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-venv');
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-setuptools');
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-wheel');
            }
            ...
            foreach ($this->pythonVersions as $pythonVersion) {
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion);
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-devel');
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-venv');
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-setuptools');
                $commands[] = $packageManager->getInstallCommand('python' . $pythonVersion . '-wheel');
            }
